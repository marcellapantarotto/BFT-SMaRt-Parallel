    ...
    private boolean early = false;
     ...
     public static void main(String[] args) {
        ...
        boolean hb = Boolean.parseBoolean(args[7]); //hybrid ?
         ...
         new LocalExecution(entries,lt,np,pG,pW,numReq,cb,hb);
     }
    
    public LocalExecution(int entries, int lateThreads, int numberPartitions, int pG, int pW, int numReqs, boolean cbase, boolean hybrid) {
        this.pg = pG;
        this.pw = pW;
        ...
        if (lateThreads > 0 && cbase == false) {
            early = true;
        }

        server = new ListServerMP(0, lateThreads, entries, numberPartitions, cbase, hybrid, false);
