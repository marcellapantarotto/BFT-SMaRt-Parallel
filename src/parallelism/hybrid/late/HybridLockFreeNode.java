 ...
 * @author eduardo
 */
public class HybridLockFreeNode extends vNode {
    ...
    public HybridLockFreeNode(Object data, Vertex vertex, ExtendedLockFreeGraph graph, int numParticions, int numConflic) {
    ...
    public void insertDepOn(HybridLockFreeNode newNode, int myPartition) {
    ...
    public int testDepMeReady() throws InterruptedException {
        ...
        for (eNode head1 : head) {
            ...
            while (ni.getVertex() != Vertex.TAIL) {
                freeNodes = freeNodes + ((HybridLockFreeNode) ni.getDependentVNode()).testReady();
                ni = ni.getNext();
            }
        }
        return freeNodes;
    ...
    public int testReady() {
        ...
        for (eNode head1 : headDepOn) {
            ...
            eNode ni = head1.getNext();
            while (ni.getVertex() != Vertex.TAIL) {
                if (!((HybridLockFreeNode) ni.getDependentVNode()).isRemoved()) {//not removed
                    /*if (remove) {
                    System.out.println(this.getAsRequest() + " not ready because: "
                            + ((HybridLockFreeNode) ni.getDependentVNode()).getAsRequest());
                }*/
                    return 0;
                }
                ni = ni.getNext();
